...
export function MegaMenuGroup({
  ...
  const { tx } = useI18n();
  const text = (value: string, dynamic?: boolean) => {
    if (dynamic || translated) return value;
    return tx(value);
  };
  return (
    <div className="megamenu-col" style={{ animationDelay: `${delayMs}ms` }}>
      <p className="px-3 text-[11px] font-bold tracking-[0.18em] text-muted-fg uppercase">
        {translated ? label : tx(label)}
      </p>
        ...
        {items.map((it) => (
          <li key={`${it.to}-${it.hash ?? it.title}`}>
            ...
            >
              ...
              />
              <span
                className="block text-[15px] font-bold text-foreground transition-colors duration-300 group-hover:text-navy"
                {...(it.dynamic ? { "data-dynamic": "" } : {})}
              >
                {text(it.title, it.dynamic)}
              </span>
              {it.desc ? (
                <span
                  className="mt-0.5 block text-[13px] leading-snug text-muted-fg transition-colors duration-300 group-hover:text-foreground/70"
                  {...(it.dynamic ? { "data-dynamic": "" } : {})}
                >
                  {text(it.desc, it.dynamic)}
                </span>
              ) : null}
            </Link>
          </li>
