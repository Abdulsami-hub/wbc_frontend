    ...
    <div className="megamenu-panel border-t border-line bg-background shadow-card">
      <div className={`container-wbc grid gap-10 py-10 lg:gap-12 lg:py-12 ${gridClass}`}>
        <div className="megamenu-col" style={{ animationDelay: "40ms" }}>
          <h2 className="text-[26px] font-bold text-foreground lg:text-[30px]">{tx(title)}</h2>
          <p className="mt-4 max-w-xs text-[16px] leading-relaxed text-muted-fg">{tx(description)}</p>
        </div>
        ...
        >
          ...
          <div className="p-5">
            <p className="text-[15px] leading-relaxed text-muted-fg">{tx(promo.text)}</p>
            <Link
              to={promo.to}
...
export function MegaMenuGroup({
  ...
  return (
    <div className="megamenu-col" style={{ animationDelay: `${delayMs}ms` }}>
      <p className="px-3 text-[11px] font-bold tracking-[0.18em] text-muted-fg uppercase">{tx(label)}</p>
